...
class RoomUpdate {
    ...
    async setup() {
        ...
        setTimeout(async () => {
            ...
            let queueKey = await mq.subscribe("ws", "onAchievementsUpdate", this.onAchievementsUpdate.bind(this), qWS2);
        }, 5000);
    }
    ...
    processPlayerExperience(game, player) {
        ...
        if (winXP > 0)
            experience.push({ type: "Win", value: winXP });
        let totalXP = experience.reduce((total, xp) => {
            total += xp.value;
            return total;
        }, 0);
        let previousPoints = Math.trunc((ws.user.level - Math.trunc(ws.user.level)) * 1000);
...
//# sourceMappingURL=onRoomUpdate.js.map