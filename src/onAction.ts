...
import { protoDecode, protoEncode } from 'acos-json-encoder';
...
class Action {
    ...
    async onClientAction(ws: any, message: any, isBinary: boolean): Promise<void> {
        profiler.StartTime("ActionUpdateLoop");

        console.log("Receiving message from Client: [" + ws?.user?.shortid + "]");
        let unsafeAction: any = null;
        try {
    ...
    async gameAction(ws: any, action: any): Promise<any> {
    ...
    async forwardAction(msg: any): Promise<void> {
        if (!msg.type) {
